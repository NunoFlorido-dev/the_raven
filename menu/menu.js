...
function preloadButtons() {
  ...
  narration = new ButtonImg(
    (width / 6) * 3.5,
    (height / 4) * 3,
    40,
    40,
    "../assets/icons/settings.png"
  ); //substituir imagem por imagem do botao do narration
  ...
  if (settings) {
    ...
    switch (menuSelected) {
      case 0:
        ...
        rect(
          (width / 3) * 1.325 + (((width / 4) * 1.15) / 9) * volume * 0.99,
          (height / 2) * 1.025,
          40,
          40
        ); //substituir por imagem do slider do volume
...
function mouseReleased() {
  if (buttons[0].isPressed(mouseX, mouseY)) {
    //clicked settings
    credits = false;
    settings = !settings;
  } else if (buttons[1].isPressed(mouseX, mouseY)) {
    //clicked credits
    settings = false;
    credits = !credits;
  }
  if (settings) {
    if (buttonsSettings[0].isPressed(mouseX, mouseY)) {
      //clicked audio settings
      menuSelected = 0;
    } else if (buttonsSettings[1].isPressed(mouseX, mouseY)) {
      //clicked keybindings settings
      menuSelected = 1;
    } else if (buttonsSettings[2].isPressed(mouseX, mouseY)) {
      //clicked visual settings
      menuSelected = 2;
    }
    if (menuSelected == 0) {
      if (buttonsAudio[0].isPressed(mouseX, mouseY)) {
        //decreased audio
        if (volume > 0) {
          volume -= 1;
        }
      } else if (buttonsAudio[1].isPressed(mouseX, mouseY)) {
        //increased audio
        if (volume < 9) {
          volume += 1;
        }
      } else if (buttonsAudio[2].isPressed(mouseX, mouseY)) {
        //clicked narration setting
